...
new_data = new_data %>%
    mutate(blocks = pop_per_segment/(300*5.7))
...
write.csv(big_blocks_sample, "C:/Users/basicuser/Desktop/East_Ucs/Big Blocks KE CSV/big_blocks_sample.csv")
...
blocks_data = read_csv("C:/Users/basicuser/Desktop/East_Ucs/KE Blocks/final_blocks.csv")
...
```
\newpage
# Final Blocks Selection
```{r, echo = FALSE, warning=FALSE, message= FALSE}

set.seed(14587)
sampled_data <- blocks_data %>% 
  group_by(uc) %>%  
  sample_n(1) %>% 
  select(Name, uc, town,  district)

print(sampled_data, n = 30)

write.csv(sampled_data, "C:/Users/basicuser/Desktop/East_Ucs/KE Blocks/sampled_blocks.csv")
...
segments_data = read_csv("C:/Users/basicuser/Desktop/East_Ucs/Segements/SEGMENT/csv.csv")

groups = segments_data %>% 
  group_by(uc) %>% 
  count()
...
set.seed(14587)
final_sampled_data <- segments_data %>% 
  group_by(uc) %>%  
  sample_n(1) %>% 
  select(name, uc, town,  district)

print(final_sampled_data, n = 30)

write.csv(final_sampled_data, "C:/Users/basicuser/Desktop/East_Ucs/KE Blocks/final_sampled_blocks.csv")
